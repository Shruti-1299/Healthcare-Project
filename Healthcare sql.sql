...
select 
	case 
        ...
        count(*) as `Age group wise total customers`
from healthcare_dataset group by `group_age` order by `group_age`;
...
select 
	case 
        ...
        count(`name`) as `Age group wise total customers`
from healthcare_dataset 
group by `group_age` 
...
select `medical condition`,count(`Name`) as `medical condition wise total patient` from healthcare_dataset
group by `medical condition` order by count(`Name`) desc limit 1;

-- Which blood group appears most frequently?
select  `Blood Type`,count(`name`) as `Most frequently occuring blood group` from healthcare_dataset 
group by `blood type` order by count(`name`)desc limit	1;
